...
initial_pcd.points = o3d.utility.Vector3dVector(initial_p)
for i in range(5):
    random_R = random_rotation_matrix()
    random_t = np.random.rand(3)
    new_point_cloud = o3d.geometry.PointCloud()
    rotated_pcd = np.asarray(initial_p) @ random_R + random_t
    ...
    sv.scene.add_point_cloud(f"restore_pcd{i}", points = restore_pcd, colors = (0,0,255),point_size = 0.01, point_shape = 'circle')
    target_pose = np.eye(4)
    target_pose[:3, :3] = rotation_mat.T
    target_pose[:3, 3] = center
    sv.scene.add_point_cloud(f"cano_pcd{i}", points = cano_pcd, colors = (0,255,0),point_size = 0.01, point_shape = 'circle')
    sv.scene.add_point_cloud(f"ini_pcd{i}", points = np.asarray(new_point_cloud.points), colors = (0,255,0),point_size = 0.01, point_shape = 'circle')
