...
def reevaluate_base_pose(object_name, arm_ip=XARM6_IP):
...
def pick(object_name='sticker', arm_ip=XARM6_IP):  
    ...
    def get_current_attach_pose_offset(object_name='sticker'):
        ...
        return grasp_offset
    ''' remain to update the hand in the planning'''
    ...
    if not mp_is_success:
        ...
        if not mp_is_success:
            ...
            # go_to_test(pose)
            status, tool_pose = xarm.get_position_se3()
            update_attach(object_name='sticker', pose=tool_pose)
            bp()
...
def insert(arm_ip=XARM6_IP, target_pose = None, mesh = None):
    ...
    # 沿末端的 z 方向平移 10cm
    offset = np.array([0, 0, -0.23])  # 平移的偏移量
