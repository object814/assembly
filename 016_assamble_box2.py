...
def get_object_pc_fp(object_name, arm_ip=XARM6_IP):
    ...
    while True:
        ...
        for rtr_dict in rtr_dict_list:
            ...
            sv.scene.add_frame("object_pose", wxyz=R.from_matrix(pose[:3, :3]).as_quat()[[3, 0, 1, 2]], position=pose[:3,3], axes_length=0.03, axes_radius=0.001)
...
def pick(object_name='box03', xarm = None):  
    ...
    if xarm.ip==XARM6_IP:
        ...
        sv_left.scene.add_frame("grasp_pose", wxyz=R.from_matrix(adjusted_rotation_matrix[:3, :3]).as_quat()[[3, 0, 1, 2]], position=pcd_center, axes_length=0.03, axes_radius=0.001)
    ...
    if xarm.ip==XARM6_IP:
        offset_grasp_in_object_frame = np.array([0, 0, +0.10]) 
...
def move_to_ee_target_from_initial(object_name = 'box01', target_pose = None, xarm = None, base_pcd =  None, switch_to_right = False):
    ...
    cano_pcd, rotation_mat, center = PointCloudUtils.canonical_bbo(initial_pcd, visualize = False, symetric=symetric, reverse_xy=reverse_xy)
...
if __name__ == "__main__":
    ...
    # rotation_path = '/home/shaol/data/zjx/rw/data/box115_1/rotation_matrix.npy'
    # center_path = '/home/shaol/data/zjx/rw/data/box115_1/center.npy'
    ...
    for step in range(6,7):
        ...
        if step == 1:
            pick(xarm = xarm_right, object_name=target_name)
            pick(xarm = xarm_left, object_name=base_name)
        else:
            pick(xarm = xarm_right, object_name=target_name)
        joint_value = base_joint_values.get(step)
        if joint_value is not None:
            xarm_left.move_by_joints_value(joint_value)
        base_pcd, target_left, target_right = specify_a_base_and_get_target_pose(object_name=base_name, arm_ip=XARM6LEFT_IP, base_pose_cam_canonical= base_pose_cam_canonical, target_pose_cam_canonical = target_pose_cam_canonical)
        ...
        move_to_ee_target_from_initial(object_name = target_name, target_pose = target_right, xarm = xarm_right, base_pcd = base_pcd, switch_to_right  = True)
        input("press enter to continue")
        xarm_right.move_gripper(open = True, wait = True)
        xarm_right.move_z_axis(distance = -0.1)
        xarm_right.move_to_home()
