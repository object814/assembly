...
from xarm7_interface import XARM7_WO_EE_URDF_PATH, XARM7_WO_EE_SRDF_PATH
...
@dataclass
class XARM7PlannerCfg:
    urdf_path: Path = XARM7_WO_EE_URDF_PATH.resolve()
    srdf_path: Path = XARM7_WO_EE_SRDF_PATH.resolve()
...
class XARM7Planner:
    def __init__(self, cfg: XARM7PlannerCfg):
        self.cfg = cfg
    ...
    def set_viewer(self):
        # Ensure that self.cfg is an instance of XARM7PlannerCfg or similar
        if not isinstance(self.cfg, XARM7PlannerCfg):
            raise TypeError("self.cfg should be an instance of XARM7PlannerCfg")
...
if __name__ == "__main__":
    ...
    ''' setup xarm '''
    xarm7_planner_cfg = XARM7PlannerCfg()
    ...
    sampled_joint_values = xarm7_planner.mplib_sample_joint_values(1000, collision_check=True)
    print(f"sys path is {sys.path}")

    for sample_id, joint_values in enumerate(sampled_joint_values):
        current_joint_values = xarm7_planner.sp_robot.get_qpos()
        planning_result = xarm7_planner.mplib_plan_qpos(current_joint_values, [joint_values])
        is_success = planning_result['status'] == 'Success'
        lgr.info(f"Sample {sample_id}: {is_success}")
        if not is_success:
            lgr.warning(f"Sample {sample_id} failed!")
        else:
            xarm7_planner.sp_follow_path(planning_result)
