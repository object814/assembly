...
# rotation_matrix = R.from_euler('y', 180, degrees=True).as_matrix() @ R.from_euler('x', 90, degrees=True).as_matrix()
# # left beblow
rotation_matrix = R.from_euler('x', -160, degrees=True).as_matrix()
# rotation_matrix = R.from_euler('z', 180, degrees=True).as_matrix() @R.from_euler('y', 180, degrees=True).as_matrix() 

X_BaseCamera[0:3, 0:3] = rotation_matrix
# X_BaseCamera[0:3, 0:3] = R.from_euler('x', 90+65, degrees=True).as_matrix()
# X_BaseCamera[0:3, 3] = np.array([0.5, 0.4, 0.2])
X_BaseCamera[0:3, 3] = np.array([0.6, -0.5, 0.9]) #left
...
if __name__ == '__main__':
    ...
    serial_number = '249322065186'  # '241122074374', '317222074181''317222074181'
    ...
    arm = XArm7WOEE()
    xarm7_planner_cfg = XARM7PlannerCfg(vis=False, n_env_pc=10000)
    ...
    def update_robot_trimesh_camera_and_mask(joint_values):
        xarm7_planner.sp_robot.set_qpos(joint_values)
        xarm7_planner.scene.update_render()  # sync pose from SAPIEN to renderer
