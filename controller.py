...
def controller(robot_name, q_para):
    ...
    for batch_idx in range(q_batch.shape[0]):
        ...
        for frame_name in pk_chain.get_frame_names():
            ...
            joint = frame.joint
            if joint.name.startswith('virtual'):
                continue

            if robot_name == 'allegro' or robot_name == 'allegro_manifm':
                if joint.name in ['joint_0.0', 'joint_4.0', 'joint_8.0', 'joint_13.0']:
                    continue
                link_dir = torch.tensor([0, 0, 1], dtype=torch.float32)

            elif robot_name == 'barrett':
                if joint.name in ['bh_j11_joint', 'bh_j21_joint']:
                    continue
                ...
                if joint.name in ['gripper_fingerB_knuckle', 'gripper_fingerC_knuckle']:
                    continue
                link_dir = torch.tensor([0, 0, -1], dtype=torch.float32)

            elif robot_name == 'shadowhand':
                if joint.name in ['WRJ2', 'WRJ1']:
                    continue
                if joint.name != 'THJ5':
                    link_dir = torch.tensor([0, 0, 1], dtype=torch.float32, device=q_batch.device)
                else:
                    link_dir = torch.tensor([1, 0, 0], dtype=torch.float32, device=q_batch.device)

            elif robot_name == 'leaphand':
                if joint.name in ['13']:
                    continue
                if joint.name in ['0', '4', '8']:
                    link_dir = torch.tensor([1, 0, 0], dtype=torch.float32)
                elif joint.name in ['1', '5', '9', '12', '14']:
            ...
            link_dir = link_dir.to(q_para.device)
            ...
            normal_dir = torch.cross(axis_dir, link_dir, dim=0)  # point to the direction of motion where the q increases
        ...
        for joint_name, dot in joint_dots.items():
            ...
            if robot_name == 'robotiq_3finger':  # open -> upper, close -> lower
                outer_q[idx] += 0.25 * ((outer_q[idx] - lower_q[idx]) if dot <= 0 else (upper_q[idx] - outer_q[idx]))
                inner_q[idx] -= 0.15 * ((upper_q[idx] - inner_q[idx]) if dot <= 0 else (inner_q[idx] - lower_q[idx]))
            else:  # open -> lower, close -> upper
                outer_q[idx] -= 0.25 * ((outer_q[idx] - lower_q[idx]) if dot >= 0 else (upper_q[idx] - outer_q[idx]))
                inner_q[idx] += 0.25 * ((upper_q[idx] - inner_q[idx]) if dot >= 0 else (inner_q[idx] - lower_q[idx]))
