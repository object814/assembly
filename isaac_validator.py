...
from validate.controller import controller
from validate.asset_info import print_asset_info, print_actor_info
...
class IsaacValidator:
    def __init__(
        ...
        # self.sim_params.physx.max_gpu_contact_pairs = 2000000  # zhenyu: LeapHand
    ...
    def run_sim(self):
        ...
        if self.is_filter and success_num != 0:  # apply inverse object transform to robot to get new joint value
            self.gym.refresh_rigid_body_state_tensor(self.sim)
            object_pose = gymtorch.wrap_tensor(self._rigid_body_states).clone()[::self.rigid_body_num, :7][success]  # success_num, 7 (xyz + quat)
            object_transform = np.eye(4)[np.newaxis].repeat(success_num, axis=0)
            object_transform[:, :3, 3] = object_pose[:, :3]
            object_transform[:, :3, :3] = Rotation.from_quat(object_pose[:, 3:7]).as_matrix()

            self.gym.refresh_dof_state_tensor(self.sim)
            dof_states = gymtorch.wrap_tensor(self._dof_states).clone().reshape(len(self.envs), -1, 2)[success, :, 0]  # success_num, DOF (xyz + euler + joint)
            robot_transform = np.eye(4)[np.newaxis].repeat(success_num, axis=0)
            ...
            q_filtered = dof_states[:, self.isaac2urdf_order].to(torch.device('cpu'))
        else:
            q_filtered = None
