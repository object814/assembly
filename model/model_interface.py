...
from utils.depth import calculate_depth
from utils.pretrain_utils import dist2weight, infonce_loss, mean_geo_diff, mean_order

class MInterface(pl.LightningModule):
    def __init__(self, cfg, network, num_iter, epoch_idx):
        super().__init__()
        self.cfg = cfg
        ...
        # if cfg.linear_kl_weight:
        #     self.num_iter = num_iter
        #     self.num_epoch = cfg.max_epochs
    ...
    def training_step(self, batch, batch_idx):
        ...
        reldist = network_output['reldist']
        mu = network_output['mu']
        logvar = network_output['logvar']

        mlat_pc = multilateration(reldist, object_pc)
        ...
        if self.cfg.loss_r:
            loss_r = nn.L1Loss()(reldist, reldist_gt)
    ...
    def on_after_backward(self):
        ...
        if nan_flag:
            self.log("skip_NaN", True)
        else:
            self.log("skip_NaN", False)

    def on_train_epoch_end(self):
        self.epoch_idx += 1
        if self.global_rank == 0:
            print(f"Training epoch: {self.epoch_idx}")
        if self.epoch_idx % self.cfg.save_every_n_epoch == 0:
            if self.global_rank == 0:
                print(f"Saving state_dict at epoch: {self.epoch_idx}")
...
class MInterfacePretraining(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        mean_geo_error = mean_geo_diff(similarity, robot_pc_1)

        self.log("mean_geo_diff", mean_geo_error)
