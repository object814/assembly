...
class PointNet2(nn.Module):
    ...
    def __init__(self, in_dim=6, hidden_dim=128, out_dim=3):
        super().__init__()
        self.sa1 = PointNetSetAbstraction(npoint=512, radius=0.02, nsample=32, in_channel=in_dim,
                                          mlp=[hidden_dim, hidden_dim * 2], group_all=False)
        self.sa2 = PointNetSetAbstraction(npoint=128, radius=0.04, nsample=64, in_channel=hidden_dim * 2 + 3,
...
class PointNet2Cls(nn.Module):
    def __init__(self, input_feature_dim, output_dim):
        ...
        self.sa1 = PointNetSetAbstraction(npoint=1024, radius=0.02, nsample=32, in_channel=input_feature_dim + 3,
                                          mlp=[64, 64, 128], group_all=False)
        self.sa2 = PointNetSetAbstraction(npoint=128, radius=0.04, nsample=64, in_channel=128 + 3, mlp=[128, 128, 256],
                                          group_all=False)
