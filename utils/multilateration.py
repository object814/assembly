...
def multilateration(reldist, fixed_pc):
    """
    Compute the target point cloud described by RelDist & fixed_pc

    :param reldist: (B, N, N), point-wise relative distance matrix between target point cloud & fixed point cloud
    :param fixed_pc: (B, N, 3), point cloud as a reference for relative distance
    :return: (B, N, 3), the target point cloud
    """
    assert reldist.ndim == 3 and fixed_pc.ndim == 3, "multilateration() requires batch data."
    v_est_p = torch.vmap(torch.vmap(estimate_p, in_dims=(None, 0)))
    target_pc = v_est_p(fixed_pc.unsqueeze(-1), reldist)[..., 0]
    return target_pc