...
def infonce_loss(phi, phi_trans, weights=None, temperature=0.1):
    B, N, D = phi.shape
    similarity = phi @ phi_trans.transpose(-2, -1)
    ...
    # positive_similarity = exp_similarity.diagonal(dim1=-2, dim2=-1)
    # sum_exp_similarity = (weights * exp_similarity).sum(dim=-1)
    # loss = -torch.log(positive_similarity / sum_exp_similarity).mean()
