...
def se3_transform_point_cloud(P, Transform):
    ...
    P_transformed = P_prime @ Transform.transpose(-2, -1)
...
if __name__ == '__main__':
    ...
    allegro = create_hand_model('allegro')
    shadowhand = create_hand_model('shadowhand')

    batch_links_pc = [allegro.links_pc, shadowhand.links_pc]

    q_a = allegro.get_canonical_q()
    allegro_pc = allegro.get_transformed_links_pc(q_a)[:, :3]
    q_s = shadowhand.get_canonical_q()
    shadowhand_pc = shadowhand.get_transformed_links_pc(q_s)[:, :3]
    batch_pc = torch.stack([allegro_pc, shadowhand_pc], dim=0)

    batch_transform, batch_transformed_pc = compute_link_pose(batch_links_pc, batch_pc, is_train=True)
    print(batch_transform)
    print(batch_transformed_pc.shape)
